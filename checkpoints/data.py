...
def load_myself_cls(partition):

    DATA_DIR = '/home/mask/MSCNN/data-generation/resources/dataset/object_4e'
    all_data = []
    all_label = []
    for h5_name in glob.glob(os.path.join(DATA_DIR, 'pc_train_test', '*%s*.h5'%partition)):
        f = h5py.File(h5_name, 'r+')
        data = f['pc_data'][:].astype('float32')
...
def load_data_partseg(partition):
    ...
    for h5_name in file:
        ...
        # sort=[]
        #
        # label_len = np.array(label).shape
...
class ModelNet40(Dataset):
    def __init__(self, num_points, partition='train'):
        ...
        # self.data, self.label = load_myself_cls(partition)
