...
class DGCNN_cls(nn.Module):
    ...
    def forward(self, x):
        batch_size = x.size(0)
        x = get_graph_feature(x, k=self.k, d=1)
        x = self.conv1(x)
        x1 = x.max(dim=-1, keepdim=False)[0]

        x = get_graph_feature(x1, k=self.k, d=2)
        x = self.conv2(x)
...
class DGCNN_partseg(nn.Module):
    ...
    def forward(self, x, l):
        ...
        x0 = get_graph_feature(x, k=self.k, d=1)
        ...
        x = get_graph_feature(x2, k=self.k, d=5)
        x = self.conv5(x)
        x = self.conv5_1(x)
        x3 = x.max(dim=-1, keepdim=False)[0]

        x = get_graph_feature(x2+x3, k=self.k, d=1)
        x = self.conv5_2(x)
        x = self.conv5_3(x)
        x4 = x.max(dim=-1, keepdim=False)[0]

        x = get_graph_feature(x3+x4, k=self.k, d=2)
        x = self.conv5_4(x)
        x = self.conv5_5(x)
        x5 = x.max(dim=-1, keepdim=False)[0]

        x = get_graph_feature(x4+x5, k=self.k, d=5)
        x = self.conv5_6(x)
        x = self.conv5_7(x)
        x6 = x.max(dim=-1, keepdim=False)[0]

        x = get_graph_feature(x5 + x6, k=self.k, d=1)
        x = self.conv5_8(x)
        x = self.conv5_9(x)
        x7 = x.max(dim=-1, keepdim=False)[0]

        x = get_graph_feature(x6 + x7, k=self.k, d=2)
        x = self.conv5_10(x)
        x = self.conv5_11(x)
        x8 = x.max(dim=-1, keepdim=False)[0]

        x = get_graph_feature(x7 + x8, k=self.k, d=5)
        x = self.conv5_12(x)
        # x = self.conv5_13(x)
...
class DGCNN_semseg(nn.Module):
    ...
    def forward(self, x):
        ...
        x = get_graph_feature(x, k=self.k, d=1, dim9=True)
        x = self.conv1(x)
        x = self.conv2(x)
        x1 = x.max(dim=-1, keepdim=False)[0]
