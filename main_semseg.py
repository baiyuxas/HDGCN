...
import h5py
def create_h5_file(data_dir, batch_cnt, data, pred_seg, true_seg):
    f = h5py.File(os.path.join(data_dir, str(batch_cnt) + ".h5"), "w")

    f.create_dataset("pc_data", data=data)
    f.create_dataset("pred_seg", data=pred_seg)
    f.create_dataset("true_seg", data=true_seg)
    f.close()
def _init_():
...
def test(args, io):
    ...
    for test_area in range(1, 7):
        ...
        if (args.test_area == 'all') or (test_area == args.test_area):
            ...
            model = nn.DataParallel(model)
            #model.load_state_dict(torch.load('/home/mask/xas_ws/dgcnn_pytorch/checkpoints/semseg_5/models/model_5.t7'))
            ...
            for data, seg in test_loader:
                ...
                save_data.append(data.numpy().reshape(4096, 9))
                data, seg = data.to(device), seg.to(device)
                ...
                pred = seg_pred.max(dim=2)[1]
                seg_np = seg.cpu().numpy()
                pred_np = pred.detach().cpu().numpy()#(1*4096)
                ...
                test_true_seg.append(seg_np)
                test_pred_seg.append(pred_np)
                save_true_seg.append(seg_np.reshape(4096, 1))
                save_pred_seg.append(pred_np.reshape(4096, 1))

            # print(test_pred_seg[0])
            data_dir ='/home/mask/xas_ws/dgcnn_final/'
            create_h5_file(data_dir, test_area, save_data, save_true_seg, save_pred_seg)
            test_true_cls = np.concatenate(test_true_cls)
            test_pred_cls = np.concatenate(test_pred_cls)#(4358144,)1064*4096

            test_acc = metrics.accuracy_score(test_true_cls, test_pred_cls)
            avg_per_class_acc = metrics.balanced_accuracy_score(test_true_cls, test_pred_cls)
            # print(np.array(test_true_cls).size)
